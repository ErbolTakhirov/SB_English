...
class ContextBuilder:
    ...
    def _build_tables_section(self, time_period: Dict[str, Any]) -> str:
        ...
        try:
            ...
            return f"## 📊 Financial Statistics\n\n{table}\n\n**Summary:** {summary}"
        except Exception as e:
            return f"## 📊 Financial Statistics\n\n_Error loading statistics: {e}_"
    ...
    def _build_trends_section(self, time_period: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not trends.get('has_enough_data'):
                return "## 📈 Trends\n\n_Not enough data for trend analysis (minimum 3 months required)_"
            ...
            lines.append(f"\n**General Trends:**")
            lines.append(f"- Income: {trend_emoji.get(income_trend, income_trend)}")
            lines.append(f"- Expenses: {trend_emoji.get(expense_trend, expense_trend)}")
            ...
            if cat_trends:
                ...
                for cat, data in list(cat_trends.items())[:5]:
                    ...
                    lines.append(
                        ...
                        f"avg: {_format_currency(data['average'])}"
                    )
    ...
    def _build_anomalies_section(self, time_period: Dict[str, Any]) -> str:
        ...
        try:
            ...
            if not alerts:
                return "## 🔍 Anomalies\n\n_No anomalous transactions found_"
            
            lines = ["## 🔍 Anomalies and Unusual Spending"]
            ...
            if incomes.exists():
                ...
                for inc in incomes[:5]:
                    lines.append(
                        ...
                        f"{_format_currency(inc.amount)} ({inc.category}) - {inc.description or 'no description'}"
                    )
            ...
            if expenses.exists():
                ...
                for exp in expenses[:5]:
                    lines.append(
                        ...
                        f"{_format_currency(exp.amount)} ({exp.category}) - {exp.description or 'no description'}"
                    )
            
            if not incomes.exists() and not expenses.exists():
                lines.append("\n_No transactions found for the specified filters_")
    ...
    def _build_user_profile_section(self) -> str:
        ...
        try:
            ...
            if hasattr(self.user, 'teen_profile') and self.user.teen_profile.bio:
                lines.append(f"\n**Bio/Occupation:** {self.user.teen_profile.bio}")
                
            lines.append(f"\n**Activity Period:** {months_active} month(s)")
            lines.append(f"**Total Transactions:** {total_income['count'] + total_expense['count']}")
            
            if total_income['avg']:
                lines.append(f"**Avg Income:** {_format_currency(total_income['avg'])}")
            if total_expense['avg']:
                lines.append(f"**Avg Expense:** {_format_currency(total_expense['avg'])}")
            
            # Determine financial type
            if total_income['total'] and total_expense['total']:
                ratio = total_expense['total'] / total_income['total']
                if ratio < 0.5:
                    financial_type = "Saver (spends less than 50% of income)"
                elif ratio < 0.8:
                    financial_type = "Optimizer (sensible spending)"
                elif ratio < 1.0:
                    financial_type = "Balancer (on the edge)"
                else:
                    financial_type = "Spender (spending exceeds income!) ⚠️"
                
                lines.append(f"**Financial Type:** {financial_type}")
    ...
    def _build_custom_table(self, months: Dict[str, Any]) -> str:
        ...
        if not months:
            return "_No data for the specified period_"
        
        header = "| Month | Income | Expense | Balance |\n|---|---|---|---|"
        lines = [header]
